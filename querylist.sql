...
--3
select id, avaliable, parking, parkingSpace from car;
--4
...
insert into maintenance join car on car.id = miantenance.id_car(`id_car`, `cause`, `intervention`, `week`, `year`, `done`) values ({$immatriculation}, {$cause}, {$intervention}, {$week}, {$year}, {$done}) where car.id_supervisor = {$supervisor} and car.id {$immatriculation};
